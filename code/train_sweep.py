...
def main():
    ...
    sweep_count += 1
    run = wandb.init(project=wandb_args.project_name)
    run.name = f"e{wandb.config.max_epoch}_lr{wandb.config.learning_rate}_bs{wandb.config.batch_size}_wd{wandb.config.weight_decay}"
    training_args.learning_rate = wandb.config.learning_rate
    training_args.num_train_epochs = wandb.config.max_epoch
    training_args.per_device_train_batch_size = wandb.config.batch_size
    training_args.per_device_eval_batch_size = wandb.config.batch_size
    training_args.weight_decay = wandb.config.weight_decay
    dir_path = wandb.config.model_name.replace("/", "_").replace(
        "-", "_") + f"_e{wandb.config.max_epoch}_lr{wandb.config.learning_rate}_bs{wandb.config.batch_size}_wd{wandb.config.weight_decay}_{sweep_count}"
    training_args.output_dir = f"../models/{dir_path}"
...
def run_mrc(
    ...
    ### QUESTION: pad_on_right는 무얼 하는 걸까? ###
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            if len(answers["answer_start"]) == 0:
                ...
                ### QUESTION token_start_index라는게 뭘 찾는거지? CLS 토큰? ###
                token_start_index = 0
