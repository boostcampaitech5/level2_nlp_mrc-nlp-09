...
def hard_voting_with_weight():
    ...
    for i in range(len(result)):
        ...
        for j in range(len(result[i])):
            if result[i][j] in temp:
                temp[result[i][j]] += float(ensemble_lists[j][-8:-5])*0.001
            else:
                temp[result[i][j]] = float(ensemble_lists[j][-8:-5])*0.001
        final_result[keys[i]] = max(temp, key=temp.get)
    ...
    # 결과 저장
    with open("../ensemble/ensemble_hard_weight.json", "w") as f:
        json.dump(final_result, f, indent=4, ensure_ascii=False)


def soft_voting(weighted):
    path = "../ensemble/soft"
    ensemble_lists = os.listdir(path)
                    ...
                    if value[0] == answers[j][keys[i]][k]["text"]:
                        if weighted:
                            temp[index][1] += answers[j][keys[i]][k]["probability"] * \
                                (float(ensemble_lists[j][-8:-5])*0.001)
                        else:
                            temp[index][1] += answers[j][keys[i]
                                                         ][k]["probability"]
                        found = True
                        break
                if not found:
                    if weighted:
                        temp.append(
                            [answers[j][keys[i]][k]["text"],
                             answers[j][keys[i]][k]["probability"] * (float(ensemble_lists[j][-8:-5])*0.001)]
                        )
                    else:
                        temp.append(
                            [answers[j][keys[i]][k]["text"],
    ...
    # 결과 저장
    with open("../ensemble/ensemble_soft_weight.json", "w") as f:
        json.dump(final_result, f, indent=4, ensure_ascii=False)


if __name__ == "__main__":
    # hard_voting()
    hard_voting_with_weight()
    # soft_voting(weighted=True)
